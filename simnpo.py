...
from baselines.losses import sim_npo, kl, mse
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run SimNPO unlearning.")
...
def run_sim_npo(
    ...
    if args.nu > 0.0:
        path = str(SCRIPT_DIR / f"checkpoints/sim_npo/{args.bench_label}/{args.retain_loss_fn}-{args.model_name}_nu")
    else:
        path = str(SCRIPT_DIR / f"checkpoints/sim_npo/{args.bench_label}/{args.retain_loss_fn}-{args.model_name}")
