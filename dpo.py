...
from baselines.losses import dpo, kl, mse
...
class Args:
    ...
    scale = [3.0, 3.0]
    nu = 0.0  # noise magnitude (set to 0.0 to disable RNA)
    lr = 5e-5
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run DPO unlearning.")
...
def run_dpo(
    ...
    for idx in range(args.max_num_batches):
        ...
        idk_inputs = tokenizer(keyword_list[0: args.batch_size], return_tensors="pt", padding=True,  truncation=True, max_length=max_length).to(model_device)
    ...
    if args.nu > 0.0:
        path = str(SCRIPT_DIR / f"checkpoints/dpo/{args.bench_label}/{args.retain_loss_fn}-{args.model_name}_nu")
    else:
        path = str(SCRIPT_DIR / f"checkpoints/dpo/{args.bench_label}/{args.retain_loss_fn}-{args.model_name}")
...
if __name__ == "__main__":
    ...
    if args.nu > 0.0:
        save_path = SCRIPT_DIR / f"checkpoints/dpo/{args.bench_label}/{args.retain_loss_fn}-{args.model_name}_nu"
    else:
        save_path = SCRIPT_DIR / f"checkpoints/dpo/{args.bench_label}/{args.retain_loss_fn}-{args.model_name}"
    if save_path.exists():
    ...
    with open(SCRIPT_DIR / "data" / "idontknow.json", "r") as f:
