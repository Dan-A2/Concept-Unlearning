...
from baselines.utils import load_model, forward_with_cache, get_data, clear_cuda_cache, save_topic_loss_plot
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run RMU unlearning.")
...
def run_rmu(
    ...
    for idx in range(num_batches):
        ...
        unlearn_loss = torch.nn.functional.mse_loss(
            updated_forget_activations, control_vec.expand_as(updated_forget_activations)
        )
    ...
    if args.nu > 0.0:
        path = str(SCRIPT_DIR / f"checkpoints/rmu/{args.bench_label}/{args.model_name}_nu")
    else:
        path = str(SCRIPT_DIR / f"checkpoints/rmu/{args.bench_label}/{args.model_name}")
