...
from baselines.losses import ga, kl, mse
...
class Args:
    ...
    # Hyperparameters (Yao et al., 2024; Maini et al., 2024 TOFU)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run GD unlearning.")
...
def run_gd(
    ...
    if args.nu > 0.0:
        path = str(SCRIPT_DIR / f"checkpoints/gd/{args.bench_label}/{args.retain_loss_fn}-{args.model_name}_nu")
    else:
        path = str(SCRIPT_DIR / f"checkpoints/gd/{args.bench_label}/{args.retain_loss_fn}-{args.model_name}")
...
if __name__ == "__main__":
    ...
    if args.nu > 0.0:
        save_path = SCRIPT_DIR / f"checkpoints/gd/{args.bench_label}/{args.retain_loss_fn}-{args.model_name}_nu"
    else:
        save_path = SCRIPT_DIR / f"checkpoints/gd/{args.bench_label}/{args.retain_loss_fn}-{args.model_name}"
    if save_path.exists():
